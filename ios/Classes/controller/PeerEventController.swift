class PeerEventController : PeerEventObserver {
    ...
    func onTrack(track: MediaStreamTrackProxy, transceiver: RtpTransceiverProxy) {
        self.eventController.sendEvent(data: [
            "event": "onTrack",
            "track": MediaStreamTrackController(messenger: self.messenger, track: track).asFlutterResult(),
            "transceiver": RtpTransceiverController(messenger: self.messenger, transceiver: transceiver).asFlutterResult()
        ])
    }
