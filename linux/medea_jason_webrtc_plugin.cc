...
#define MEDEA_JASON_WEBRTC_PLUGIN(obj)                                     \
  (G_TYPE_CHECK_INSTANCE_CAST((obj), medea_jason_webrtc_plugin_get_type(), \
                              MedeaJasonWebrtcPlugin))
...
G_DEFINE_TYPE(MedeaJasonWebrtcPlugin,
              medea_jason_webrtc_plugin,
              g_object_get_type())

// Called when a method call is received from Flutter.
static void medea_jason_webrtc_plugin_handle_method_call(
...
static void medea_jason_webrtc_plugin_dispose(GObject* object) {
  G_OBJECT_CLASS(medea_jason_webrtc_plugin_parent_class)->dispose(object);
}

static void medea_jason_webrtc_plugin_class_init(
    MedeaJasonWebrtcPluginClass* klass) {
  G_OBJECT_CLASS(klass)->dispose = medea_jason_webrtc_plugin_dispose;
}

static void medea_jason_webrtc_plugin_init(MedeaJasonWebrtcPlugin* self) {}

static void method_call_cb(FlMethodChannel* channel,
                           FlMethodCall* method_call,
                           gpointer user_data) {
  MedeaJasonWebrtcPlugin* plugin = MEDEA_JASON_WEBRTC_PLUGIN(user_data);
  medea_jason_webrtc_plugin_handle_method_call(plugin, method_call);
}

void medea_jason_webrtc_plugin_register_with_registrar(
    FlPluginRegistrar* registrar) {
  MedeaJasonWebrtcPlugin* plugin = MEDEA_JASON_WEBRTC_PLUGIN(
      g_object_new(medea_jason_webrtc_plugin_get_type(), nullptr));
