...
enum class IceConnectionState(val value: Int) {
     ...
     * calls to `PeerConnection.addIceCandidate()` (or both).
     */
    NEW(0),
     ...
     * found a pair which will allow the peer connection to be made. It is possible that gathering
     * of candidates is also still underway.
     */
    CHECKING(1),

    /**
     * A usable pairing of local and remote candidates has been found for all components of
     * the connection, and the connection has been established. It is possible that gathering
     * is still underway, and it is also possible that the ICE agent is still checking candidates
    ...
    /**
     * The ICE candidate has checked all candidates pairs against one another and has failed to
     * find compatible matches for all components of the connection. It is, however, possible that
     * the ICE agent did find compatible connections for some components.
     */
    FAILED(4),

    /**
     * Checks to ensure that components are still connected failed for at least one component of
     * the `PeerConnection`. This is a less stringent test than failed and may trigger
     * intermittently and resolve just as spontaneously on less reliable networks, or during
     * temporary disconnections. When the problem resolves, the connection may return to
     * the connected state.
     */
    DISCONNECTED(5),
