...
public class GetUserMediaImpl {
  ...
  @Nullable
  private String findVideoCapturer(CameraEnumerator enumerator, boolean isFacing, String sourceId) {
  ...
  private String getSourceIdConstraint(@Nullable ConstraintsMap mediaConstraints) {
    if (mediaConstraints != null
        ...
        && mediaConstraints.getType("optional") == ObjectType.Map) {
   ...
   */
  void getUserMedia(
      @NonNull final ConstraintsMap constraints,
  ...
  private VideoTrack getUserVideo(@NonNull ConstraintsMap constraints) {
    ...
    VideoCapturer videoCapturer =
        cameraEnumerator.createCapturer(deviceId, new CameraEventsHandler());
  ...
  private void requestPermissions(
    ...
    PermissionUtils.requestPermissions(
        context, activity, permissions.toArray(new String[0]), callback);
  }
  ...
  void disposeSource(String trackId) {
    ...
    if (capturerInfo != null) {
      try {
        ...
        Log.d(TAG, "Tries to dispose already disposed VideoCapturer");
      }
  ...
  void hasTorch(String trackId, @NonNull Result result) {
    ...
    if (VERSION.SDK_INT >= VERSION_CODES.LOLLIPOP && info.capturer instanceof Camera2Capturer) {
      ...
      try {
        ...
        resultError(
            "hasTorch",
            "[TORCH] Failed to get `" + e.fieldName + "` from `" + e.className + "`",
            result);
        return;
      }
  ...
  @Nullable
  private Object getPrivateProperty(
      @NonNull Class<?> klass, Object object, @NonNull String fieldName)
  ...
  private static class NoSuchFieldWithNameException extends NoSuchFieldException {
    ...
    NoSuchFieldWithNameException(
        String className, String fieldName, @NonNull NoSuchFieldException e) {
      super(e.getMessage());
  ...
  public void reStartCamera(@NonNull IsCameraEnabled getCameraId) {
    for (Map.Entry<String, VideoCapturerInfo> item : mVideoCapturers.entrySet()) {
      if (!item.getValue().isScreenCapture && getCameraId.isEnabled(item.getKey())) {
        item.getValue()
            .capturer
            .startCapture(item.getValue().width, item.getValue().height, item.getValue().fps);
      }
    }
  }
