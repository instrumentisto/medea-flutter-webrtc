...
import com.cloudwebrtc.webrtc.utils.MethodCallObserver;
...
public class MethodCallHandlerImpl implements MethodCallHandler, StateProvider, MethodCallObserver {
  ...
  private final Map<String, MethodCallObserver> methodcallObservers = new HashMap<>();
  ...
  private void ensureInitialized() {
    ...
    getUserMediaImpl.audioDeviceModule = audioDeviceModule;
  ...
  @Override
  public void observerMethodCall(MethodCall call, @NonNull AnyThreadResult result) {
    switch (call.method) {
    }
  }
  ...
  public void onMethodCall(MethodCall call, @NonNull Result notSafeResult) {
    ...
    final String observerId = call.argument("observerId");
    if (observerId == null) {
      observerMethodCall(call, result);
    } else {
      final MethodCallObserver observer = methodcallObservers.get(observerId);
      if (observer == null) {
        resultError(
            call.method, "MethodCallObserver with a provided observerId isn't found", result);
      } else {
        observer.observerMethodCall(call, result);
      }
    }
      ...
      case "videoRendererSetSrcObject":
        {
          ...
          if (track == null || !track.kind().equals("video")) {
            resultError(
                ...
                "provided invalid VideoTrack [" + trackId + "]",
                result);
            return;
          }
  ...
  public String peerConnectionInit(ConstraintsMap configuration, ConstraintsMap constraints) {
    ...
    PeerConnectionObserver observer =
        new PeerConnectionObserver(conf, this, messenger, peerConnectionId, getUserMediaImpl);
    PeerConnection peerConnection = mFactory.createPeerConnection(conf, observer);
  ...
  public void peerConnectionSetConfiguration(
      ConstraintsMap configuration, PeerConnection peerConnection) {
    if (peerConnection == null) {
  ...
  public void peerConnectionCreateOffer(
      String id, ConstraintsMap constraints, final Result result) {
  ...
  public void peerConnectionSetLocalDescription(
      ConstraintsMap sdpMap, final String id, final Result result) {
    PeerConnection peerConnection = getPeerConnection(id);
    if (peerConnection != null) {
  ...
  public void peerConnectionSetRemoteDescription(
      final ConstraintsMap sdpMap, final String id, final Result result) {
    PeerConnection peerConnection = getPeerConnection(id);
    if (peerConnection != null) {
      SessionDescription sdp =
          new SessionDescription(
  ...
  public void peerConnectionAddICECandidate(
      ConstraintsMap candidateMap, final String id, final Result result) {
    boolean res = false;
    PeerConnection peerConnection = getPeerConnection(id);
    if (peerConnection != null) {
  ...
  public void addTransceiver(
      String peerConnectionId, String trackId, Map<String, Object> transceiverInit, Result result) {
  ...
  public void addTransceiverOfType(
      String peerConnectionId,
      String mediaType,
      Map<String, Object> transceiverInit,
      Result result) {
    PeerConnectionObserver pco = mPeerConnectionObservers.get(peerConnectionId);
    if (pco == null || pco.getPeerConnection() == null) {
  ...
  public void rtpTransceiverSetDirection(
      String peerConnectionId, String direction, int transceiverId, Result result) {
    PeerConnectionObserver pco = mPeerConnectionObservers.get(peerConnectionId);
    if (pco == null || pco.getPeerConnection() == null) {
  ...
  public void rtpTransceiverGetDirection(
      String peerConnectionId, int transceiverId, Result result) {
    PeerConnectionObserver pco = mPeerConnectionObservers.get(peerConnectionId);
    if (pco == null || pco.getPeerConnection() == null) {
      resultError("rtpTransceiverSetDirection", "peerConnection is null", result);
    } else {
  ...
  public void rtpTransceiverGetCurrentDirection(
      String peerConnectionId, int transceiverId, Result result) {
    PeerConnectionObserver pco = mPeerConnectionObservers.get(peerConnectionId);
    if (pco == null || pco.getPeerConnection() == null) {
      resultError("rtpTransceiverSetDirection", "peerConnection is null", result);
  ...
  public void rtpSenderSetParameters(
      String peerConnectionId, String rtpSenderId, Map<String, Object> parameters, Result result) {
    PeerConnectionObserver pco = mPeerConnectionObservers.get(peerConnectionId);
    if (pco == null || pco.getPeerConnection() == null) {
  ...
  public void rtpSenderSetTrack(
      String peerConnectionId, String rtpSenderId, String trackId, boolean replace, Result result) {
    PeerConnectionObserver pco = mPeerConnectionObservers.get(peerConnectionId);
    if (pco == null || pco.getPeerConnection() == null) {
