...
constexpr float kInt16MaxAsFloat =
...
// `SCStreamOutput` that forwards audio buffers to C++ `SysAudioSource`
// instance.
...
- (void)stream:(SCStream*)stream didStopWithError:(NSError *)error {
  if (error) {
    RTC_LOG(LS_ERROR) << "Stream stopped with error: " << error.code;
  } else {
...
- (void)stream:(SCStream*)stream didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer ofType:(SCStreamOutputType)type {
  if (type != SCStreamOutputTypeAudio) {
    RTC_LOG(LS_VERBOSE) << "Ignoring non-audio output type: " << (int)type;
    return;
  }
  ...
  if (!fmt) {
    RTC_LOG(LS_ERROR) << "Missing `CMFormatDescription` from `CMSampleBuffer`";
    return;
  }
  ...
  if (!asbd) {
    RTC_LOG(LS_ERROR) << "Missing `AudioStreamBasicDescription`";
    return;
  }
  ...
  if (asbd->mFormatID != kAudioFormatLinearPCM) {
    RTC_LOG(LS_ERROR)
        ...
        << "`) expected `kAudioFormatLinearPCM`";
    return;
  }
  ...
  if (channels == 0) {
    RTC_LOG(LS_ERROR) << "`AudioStreamBasicDescription` reports zero channels";
    return;
  }
  ...
  if (isNonInterleaved) {
    ...
    const size_t ablSize =
        sizeof(AudioBufferList) + (channels - 1) * sizeof(AudioBuffer);
    std::vector<uint8_t> ablStorage(ablSize);
    AudioBufferList* abl =
        reinterpret_cast<AudioBufferList*>(ablStorage.data());
    ...
    if (isFloat) {
      ...
      for (CMItemCount i = 0; i < totalFrames; ++i) {
        ...
        for (UInt32 c = 0; c < channels; ++c) {
          const float* src =
              reinterpret_cast<const float*>(abl->mBuffers[c].mData);
          acc += src[i];
        }
      ...
      if (channels == 1) {
        ...
        const int16_t* src =
            reinterpret_cast<const int16_t*>(abl->mBuffers[0].mData);
        ...
        for (CMItemCount i = 0; i < totalFrames; ++i) {
          ...
          for (UInt32 c = 0; c < channels; ++c) {
            const int16_t* src =
                reinterpret_cast<const int16_t*>(abl->mBuffers[c].mData);
            acc += src[i];
          }
    ...
    if (self.owner) {
      self.owner->OnPcmDataFromSC(mono.data(),
                                  (size_t)totalFrames,
                                  1,
                                  asbd->mSampleRate);
    }
    ...
    if (getPtrStatus != kCMBlockBufferNoErr) {
      RTC_LOG(LS_ERROR)
          << "`CMBlockBufferGetDataPointer` failed with status: "
          << (int)getPtrStatus;
      return;
    }
...
API_AVAILABLE(macos(13.0)) bool SysAudioSource::StartCapture() {
  ...
  @autoreleasepool {
    ...
    // Wait up to `kShareableContentTimeoutSeconds` max.
    long waitResult = dispatch_semaphore_wait(sema,
                                              dispatch_time(DISPATCH_TIME_NOW,
                                              kShareableContentTimeoutSeconds *
                                                  NSEC_PER_SEC));
    if (waitResult != 0) {
      RTC_LOG(LS_ERROR)
    ...
    dispatch_queue_t q = dispatch_queue_create("SysAudioSource.SCStreamOutput",
                                               DISPATCH_QUEUE_SERIAL);
    sample_queue_ = (__bridge_retained void*)q;

    NSError *addStreamOutputError = nil;
    BOOL add_output_ok = [stream addStreamOutput:output
    ...
    if (!add_output_ok) {
      RTC_LOG(LS_ERROR)
          << "`SysAudioSource(Mac)`: "
          << "`addStreamOutput(SCStreamOutputTypeAudio)` error: "
    ...
    [stream startCaptureWithCompletionHandler:^(NSError *startError) {
      if (startError) {
        RTC_LOG(LS_ERROR) << "`SysAudioSource(Mac)`: `startCapture()` error: "
                          << startError.code;
...
API_AVAILABLE(macos(13.0)) void SysAudioSource::StopCapture() {
  ...
  @autoreleasepool {
    ...
    // Keep local strong references for async completion, but `null` out members
    // now.
    ...
    if (stream) {
      [stream stopCaptureWithCompletionHandler:^(NSError *_Nullable error) {
        if (error && error.code != SCStreamErrorAttemptToStopStreamState) {
          RTC_LOG(LS_ERROR)
              << "`SysAudioSource(Mac)`: "
              << "`stopCaptureWithCompletionHandler` error: "
              << error.code;
        }
...
bool SysAudioSource::ProcessRecordedPart(bool /*firstInCycle*/) {
  ...
  if (recorded_samples_.size() >= kRecordingPart) {
    source_->OnData(recorded_samples_.data(),
                    kBitsPerSample,
                    kRecordingFrequency,
                    kRecordingChannels,
                    kRecordingPart);
    recorded_samples_.erase(recorded_samples_.begin(),
