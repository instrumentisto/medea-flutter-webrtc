...
namespace webrtc {
class WrapAudioDeviceModuleInterface
    : public rtc::RefCountedObject<AudioDeviceModule> {
 public:
  // Creates local `AudioDeviceModule` on worker thread , for
  // uses methods of the default `AudioDeviceModule` implementation.
  //
  // `create_local_adm` must be called before all methods.
  virtual void create_local_adm(
      webrtc::AudioDeviceModule::AudioLayer audio_layer,
      webrtc::TaskQueueFactory* task_queue_factory) = 0;
};

class WrapAudioDeviceModule : public WrapAudioDeviceModuleInterface {
 public:
  // Creates local `AudioDeviceModule` on worker thread , for
  // uses methods of the default `AudioDeviceModule` implementation.
  //
  // `create_local_adm` must be called before all methods.
  void create_local_adm(webrtc::AudioDeviceModule::AudioLayer audio_layer,
...
// Define proxy for WrapAudioDeviceModuleInterface.
BEGIN_PROXY_MAP(WrapAudioDeviceModule)
...
#endif  // WEBRTC_IOS
END_PROXY_MAP(WrapAudioDeviceModule)
}  // namespace webrtc
